...
trellis_status trellis_safetensors_read_f32(
    ...
    if (elem_size == 0 || meta->data_end < meta->data_begin ||
        meta->data_end - meta->data_begin != n * elem_size) {
        return TRELLIS_STATUS_PARSE_ERROR;
    }
    ...
    for (uint64_t i = 0; i < n; ++i) {
        if (meta->dtype == TRELLIS_DTYPE_F32) {
            float v;
            if (fread(&v, 1, 4, f) != 4) { status = TRELLIS_STATUS_IO_ERROR; break; }
            dst[i] = v;
        } else if (meta->dtype == TRELLIS_DTYPE_F16) {
            ggml_fp16_t v;
            if (fread(&v, 1, 2, f) != 2) { status = TRELLIS_STATUS_IO_ERROR; break; }
            dst[i] = ggml_fp16_to_fp32(v);
        } else if (meta->dtype == TRELLIS_DTYPE_BF16) {
            ggml_bf16_t v;
            if (fread(&v, 1, 2, f) != 2) { status = TRELLIS_STATUS_IO_ERROR; break; }
            dst[i] = ggml_bf16_to_fp32(v);
        } else if (meta->dtype == TRELLIS_DTYPE_I64) {
            int64_t v;
            if (fread(&v, 1, 8, f) != 8) { status = TRELLIS_STATUS_IO_ERROR; break; }
            dst[i] = (float) v;
            ...
            unsigned char v;
            if (fread(&v, 1, 1, f) != 1) { status = TRELLIS_STATUS_IO_ERROR; break; }
            dst[i] = (float) v;
        } else {
