...
static trellis_status bind_sparse_conv3d_flex(
     ...
     * ggml [in,3,3,3*out] while preserving raw contiguous storage.
     */
...
static trellis_status bind_convnext_block(
    ...
    const char * prefix,
    int channels,
    trellis_shape_decoder_convnext_block_weights * block,
    char * first_issue,
    size_t first_issue_size) {
    char name[512];
    char bias[512];
...
static trellis_status bind_c2s_block(
    ...
    int in_channels,
    int out_channels,
    trellis_shape_decoder_c2s_block_weights * block,
    char * first_issue,
    size_t first_issue_size) {
    char name[512];
    char bias[512];
...
trellis_status trellis_shape_decoder_bind_weights(
    ...
    const int channels[TRELLIS_SHAPE_DECODER_LEVELS] = {1024, 512, 256, 128, 64};
    const int blocks[TRELLIS_SHAPE_DECODER_LEVELS] = {4, 16, 8, 4, 0};
    weights->levels = TRELLIS_SHAPE_DECODER_LEVELS;
    for (int i = 0; i < TRELLIS_SHAPE_DECODER_LEVELS; ++i) {
    ...
    for (int level = 0; level < TRELLIS_SHAPE_DECODER_LEVELS; ++level) {
        ...
        if (level < TRELLIS_SHAPE_DECODER_LEVELS - 1) {
    ...
    return bind_linear(
        ...
        "output_layer.weight",
        "output_layer.bias",
        channels[TRELLIS_SHAPE_DECODER_LEVELS - 1],
        7,
        &weights->output_w,
        &weights->output_b,
        first_issue,
