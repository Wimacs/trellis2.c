...
trellis_status trellis_dino_vit_forward_f32_host(
    ...
    int n_tokens,
    float ** tokens_out) {
    if (cuda == NULL || cuda->backend == NULL || weights == NULL || tokens == NULL || tokens_out == NULL ||
        batch <= 0 || n_tokens <= 0 ||
    ...
    *tokens_out = NULL;
    ...
    struct ggml_tensor * x = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, weights->hidden_size, n_tokens, batch);
    struct ggml_tensor * cos_phase = NULL;
    struct ggml_tensor * sin_phase = NULL;
    if (cos_phase_data != NULL) {
        cos_phase = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, weights->head_dim, n_tokens, 1, 1);
        sin_phase = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, weights->head_dim, n_tokens, 1, 1);
    }
    struct ggml_tensor * y = trellis_dino_vit_forward(ctx, x, cos_phase, sin_phase, weights);
    if (x == NULL || (cos_phase_data != NULL && (cos_phase == NULL || sin_phase == NULL)) || y == NULL) {
        ggml_free(ctx);
    ...
    ggml_backend_tensor_set(x, tokens, 0, ggml_nbytes(x));
    if (cos_phase_data != NULL) {
