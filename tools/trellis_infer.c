...
static void usage(const char * argv0) {
    fprintf(stderr,
        ...
        "Current subcommands load safetensors weights into ggml CUDA tensors; graph\n"
        "forward assembly is being added here stage by stage.\n"
        "\n"
        "Options:\n"
        ...
        "  --dry-dino-patch         Run a real-weight DINO patch embedding smoke test\n"
...
static int load_decoder_store(
    ...
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 sparse-structure decoder: open %s failed: %s\n", path, trellis_status_string(status));
        return 1;
    }
    size_t graph_tensors = st.n_tensors + 64;
    trellis_safetensors_close(&st);

    trellis_tensor_store store;
    status = trellis_tensor_store_init(&store, graph_tensors, 0);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 sparse-structure decoder: tensor store init failed: %s\n", trellis_status_string(status));
        return 1;
    }
    size_t loaded = 0;
    status = trellis_tensor_store_load_safetensors_f32(&store, cuda, path, false, &loaded);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 sparse-structure decoder: load failed: %s\n", trellis_status_string(status));
    ...
    char issue[256];
    status = trellis_ss_decoder_bind_weights(&store, &weights, issue, sizeof(issue));
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 sparse-structure decoder: bind failed: %s", trellis_status_string(status));
        if (issue[0] != '\0') {
    ...
    printf("stage1 sparse-structure decoder: loaded %zu tensors into CUDA ggml store\n", loaded);
    printf("  bindings ok: input 8->512, middle=2, upsample=2, output 32->1\n");
    if (dry_decode) {
        ...
        float * logits = NULL;
        int output_size = 0;
        status = trellis_ss_decoder_forward_f32_host(&weights, latent, cuda->device, dry_batch, dry_latent_size, &logits, &output_size);
        free(latent);
        if (status != TRELLIS_STATUS_OK) {
...
static int load_dino_store(
    ...
    if (!make_join_path(dino_dir, "model.safetensors", path, sizeof(path))) {
        fprintf(stderr, "dino: invalid model path\n");
        return 1;
    }
    ...
    trellis_status status = trellis_safetensors_open(path, &st);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "dino: open %s failed: %s\n", path, trellis_status_string(status));
        return 1;
    }
    size_t graph_tensors = st.n_tensors + 64;
    trellis_safetensors_close(&st);
    ...
    status = trellis_tensor_store_init(&store, graph_tensors, 0);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "dino: tensor store init failed: %s\n", trellis_status_string(status));
        return 1;
    }

    size_t loaded = 0;
    status = trellis_tensor_store_load_safetensors_f32(&store, cuda, path, true, &loaded);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "dino: load failed: %s\n", trellis_status_string(status));
        trellis_tensor_store_free(&store);
        return 1;
    }
    ...
    char issue[256];
    status = trellis_dino_vit_bind_weights(&store, &weights, issue, sizeof(issue));
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "dino: bind failed: %s", trellis_status_string(status));
        if (issue[0] != '\0') {
            fprintf(stderr, " (%s)", issue);
        }
        fprintf(stderr, "\n");
    ...
    printf("dino: loaded %zu tensors into CUDA ggml store\n", loaded);
    printf("  bindings ok: layers=%d hidden=%d heads=%d head_dim=%d patch=%d registers=%d\n",
    ...
    if (dry_dino_patch) {
        ...
        float * tokens = NULL;
        int n_patches = 0;
        status = trellis_dino_patch_embed_f32_host(&weights, image, cuda->device, BATCH, IMAGE_H, IMAGE_W, &tokens, &n_patches);
        free(image);
        if (status != TRELLIS_STATUS_OK) {
            fprintf(stderr, "dino: dry patch failed: %s\n", trellis_status_string(status));
            free(tokens);
            trellis_tensor_store_free(&store);
            return 1;
        }
        printf("  dry patch ok: image=%dx%d patches=%d hidden=%d first=",
            IMAGE_H,
            IMAGE_W,
            n_patches,
            weights.hidden_size);
...
static int load_flow_for_stage1(
    ...
    fprintf(stderr, "stage1 image: loading sparse-structure flow from %s\n", path);
    trellis_safetensors st;
    trellis_status status = trellis_safetensors_open(path, &st);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: open flow failed: %s\n", trellis_status_string(status));
        return 0;
    }
    size_t graph_tensors = st.n_tensors + 64;
    trellis_safetensors_close(&st);
    status = trellis_tensor_store_init(store, graph_tensors, 0);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: flow store init failed: %s\n", trellis_status_string(status));
        return 0;
    }
    status = trellis_tensor_store_load_safetensors_f32(store, cuda, path, true, NULL);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: flow load failed: %s\n", trellis_status_string(status));
        return 0;
    }
    char issue[256];
    status = trellis_ss_flow_bind_weights(store, weights, issue, sizeof(issue));
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: flow bind failed: %s%s%s\n",
            trellis_status_string(status), issue[0] == '\0' ? "" : " ", issue);
        return 0;
    }
    fprintf(stderr, "stage1 image: sparse-structure flow ready blocks=%d channels=%d\n",
...
static int load_decoder_for_stage1(
    ...
    fprintf(stderr, "stage1 image: loading sparse-structure decoder from %s\n", path);
    trellis_safetensors st;
    trellis_status status = trellis_safetensors_open(path, &st);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: open decoder failed: %s\n", trellis_status_string(status));
        return 0;
    }
    size_t graph_tensors = st.n_tensors + 64;
    trellis_safetensors_close(&st);
    status = trellis_tensor_store_init(store, graph_tensors, 0);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: decoder store init failed: %s\n", trellis_status_string(status));
        return 0;
    }
    status = trellis_tensor_store_load_safetensors_f32(store, cuda, path, false, NULL);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: decoder load failed: %s\n", trellis_status_string(status));
        return 0;
    }
    char issue[256];
    status = trellis_ss_decoder_bind_weights(store, weights, issue, sizeof(issue));
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 smoke: decoder bind failed: %s%s%s\n",
            trellis_status_string(status), issue[0] == '\0' ? "" : " ", issue);
        return 0;
    }
    fprintf(stderr, "stage1 image: sparse-structure decoder ready\n");
    return 1;
}
...
static int load_dino_for_stage1(
    ...
    char path[4096];
    if (!make_join_path(dino_dir, "model.safetensors", path, sizeof(path))) {
        fprintf(stderr, "stage1 image: invalid dino model path\n");
        return 0;
    }
    fprintf(stderr, "stage1 image: loading DINO weights from %s\n", path);
    trellis_safetensors st;
    trellis_status status = trellis_safetensors_open(path, &st);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 image: open dino failed: %s\n", trellis_status_string(status));
        return 0;
    }
    size_t graph_tensors = st.n_tensors + 64;
    trellis_safetensors_close(&st);
    status = trellis_tensor_store_init(store, graph_tensors, 0);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 image: dino store init failed: %s\n", trellis_status_string(status));
        return 0;
    }
    status = trellis_tensor_store_load_safetensors_f32(store, cuda, path, true, NULL);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 image: dino load failed: %s\n", trellis_status_string(status));
        return 0;
    }
    char issue[256];
    status = trellis_dino_vit_bind_weights(store, weights, issue, sizeof(issue));
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 image: dino bind failed: %s%s%s\n",
            trellis_status_string(status), issue[0] == '\0' ? "" : " ", issue);
        return 0;
    }
    fprintf(stderr, "stage1 image: DINO weights ready\n");
    return 1;
}
...
static int run_dino_condition(
    ...
    *cond_tokens_out = 0;
    ...
    int rc = 0;
    float * image = NULL;
    float * patch_tokens = NULL;
    float * input_tokens = NULL;
    float * cls = NULL;
    float * regs = NULL;
    ...
    fprintf(stderr, "stage1 image: preprocessed %s from %dx%d to %dx%d\n",
    ...
    int n_patches = 0;
    fprintf(stderr, "stage1 image: running DINO patch embedding\n");
    trellis_status status = trellis_dino_patch_embed_f32_host(
        dino, image, cuda->device, 1, cond_resolution, cond_resolution, &patch_tokens, &n_patches);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 image: dino patch embedding failed: %s\n", trellis_status_string(status));
        goto cleanup;
    }
    shape_3d(shape, sizeof(shape), 1, n_patches, dino->hidden_size);
    if (!check_or_dump_tensor_f32(dump, "dino_patch_tokens", shape, patch_tokens,
            (size_t) n_patches * (size_t) dino->hidden_size)) {
        goto cleanup;
    }
    ...
    input_tokens = (float *) malloc(token_count * sizeof(float));
    cls = (float *) malloc((size_t) dino->hidden_size * sizeof(float));
    regs = (float *) malloc((size_t) dino->register_tokens_count * (size_t) dino->hidden_size * sizeof(float));
    ...
    if (input_tokens == NULL || cls == NULL || regs == NULL || cos_phase == NULL || sin_phase == NULL) {
        fprintf(stderr, "stage1 image: dino host allocation failed\n");
        goto cleanup;
    }
    if (!copy_tensor_f32(dino->cls_token, cls, (size_t) dino->hidden_size) ||
        !copy_tensor_f32(dino->register_tokens, regs, (size_t) dino->register_tokens_count * (size_t) dino->hidden_size)) {
        fprintf(stderr, "stage1 image: failed to copy dino special tokens\n");
        goto cleanup;
    }
    memcpy(input_tokens, cls, (size_t) dino->hidden_size * sizeof(float));
    memcpy(input_tokens + (size_t) dino->hidden_size, regs,
        (size_t) dino->register_tokens_count * (size_t) dino->hidden_size * sizeof(float));
    memcpy(input_tokens + (size_t) n_special * (size_t) dino->hidden_size, patch_tokens,
        (size_t) n_patches * (size_t) dino->hidden_size * sizeof(float));
    shape_3d(shape, sizeof(shape), 1, total_tokens, dino->hidden_size);
    if (!check_or_dump_tensor_f32(dump, "dino_input_tokens", shape, input_tokens, token_count)) {
        goto cleanup;
    }

    status = trellis_dino_rope_2d_phases_f32(
        n_special,
        patches_h,
        patches_w,
        dino->head_dim,
    ...
    fprintf(stderr, "stage1 image: running DINO transformer tokens=%d layers=%d\n",
        total_tokens, TRELLIS_DINO_VIT_LAYERS);
    status = trellis_dino_vit_forward_f32_host(
        cuda, dino, input_tokens, cos_phase, sin_phase, 1, total_tokens, &output_tokens);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "stage1 image: dino transformer failed: %s\n", trellis_status_string(status));
        goto cleanup;
    }
    shape_3d(shape, sizeof(shape), 1, total_tokens, dino->hidden_size);
    ...
    printf("stage1 image: DINO %s %dx%d -> %dx%d patches=%d tokens=%d hidden=%d\n",
...
static int run_stage1_image_voxels(
    ...
    if (input_cond_path != NULL) {
        ...
        printf("stage1 image: using reference cond %s tokens=%d hidden=1024\n", input_cond_path, cond_tokens);
    } else {
    ...
    if (flow_blocks_override >= 0) {
        ...
        printf("stage1 image: debug flow blocks override=%d\n", flow.n_blocks);
    }
    if (flow_block_parts_override >= 0) {
        ...
        printf("stage1 image: debug flow block parts override=%d\n", flow.debug_block_parts);
    }
    if (flow_no_rope) {
        flow.debug_disable_rope = 1;
        printf("stage1 image: debug flow RoPE disabled\n");
    }
    ...
    if (input_latent_path != NULL) {
        ...
        printf("stage1 image: using reference latent %s layout=NCDHW -> token-major\n", input_latent_path);
    } else {
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        fprintf(stderr, "stage1 image: step %d/%d running conditional flow t=%.6g\n", step + 1, steps, t);
        ...
        fprintf(stderr, "stage1 image: step %d/%d running unconditional flow t=%.6g\n", step + 1, steps, t);
        ...
        float * logits = NULL;
        int output_size = 0;
        fprintf(stderr, "stage1 image: step %d/%d decoding sparse structure\n", step + 1, steps);
        status = trellis_ss_decoder_forward_f32_host(&decoder, latent_ncdhw, cuda->device, 1, latent_size, &logits, &output_size);
        if (status != TRELLIS_STATUS_OK) {
        ...
        free(frame_logits);
        free(logits);
        printf("stage1 image: step %d/%d t=%.6g -> %.6g voxels=%d at %d^3 logits64[min=%.6g mean=%.6g max=%.6g]\n",
            step + 1, steps, t, t_prev, voxel_count, frame_resolution, logit_min, logit_mean, logit_max);
        fflush(stdout);
    }

    if (out_dir != NULL && out_dir[0] != '\0') {
        printf("stage1 image: wrote %d decoded voxel frames to %s (threshold=%.6g)\n", steps, out_dir, threshold);
    } else {
        printf("stage1 image: decoded %d voxel frames (threshold=%.6g)\n", steps, threshold);
    }
    fflush(stdout);
    rc = 0;
...
static int run_stage1_smoke_voxels(
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        trellis_flow_euler_step_f32(latent, pred, latent_count, 1e-5f, t, t_prev, next, x0);
        memcpy(latent, next, latent_count * sizeof(float));
        printf("stage1 smoke: step %d/%d t=%.6g -> %.6g\n", step + 1, steps, t, t_prev);
    }

    float * logits = NULL;
    int output_size = 0;
    status = trellis_ss_decoder_forward_f32_host(&decoder, latent, cuda->device, 1, latent_size, &logits, &output_size);
    if (status != TRELLIS_STATUS_OK) {
    ...
    printf("stage1 smoke: wrote %d voxels at %d^3 to %s (threshold=%.6g)\n",
...
static int load_store(
    ...
    trellis_status status = trellis_safetensors_open(path, &st);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "%s: open %s failed: %s\n", label, path, trellis_status_string(status));
        return 1;
    }
    size_t graph_tensors = st.n_tensors + 64;
    trellis_safetensors_close(&st);
    ...
    status = trellis_tensor_store_init(&store, graph_tensors, 0);
    if (status != TRELLIS_STATUS_OK) {
        fprintf(stderr, "%s: tensor store init failed: %s\n", label, trellis_status_string(status));
        return 1;
    }
    size_t loaded = 0;
    status = trellis_tensor_store_load_safetensors_f32(&store, cuda, path, true, &loaded);
    ...
    printf("%s: loaded %zu tensors into CUDA ggml store\n", label, loaded);
    if (input_w != NULL && input_b != NULL) {
        printf("  input_layer.weight ne=[%lld,%lld] input_layer.bias ne=[%lld]\n",
            (long long) input_w->ne[0],
            (long long) input_w->ne[1],
            (long long) input_b->ne[0]);
    }
    if (out_w != NULL) {
        printf("  out_layer.weight ne=[%lld,%lld]\n",
    ...
    if (bind_fn != NULL) {
        trellis_dit_flow_weights weights;
        char issue[256];
        status = bind_fn(&store, &weights, issue, sizeof(issue));
        if (status != TRELLIS_STATUS_OK) {
            fprintf(stderr, "%s: bind failed: %s", label, trellis_status_string(status));
            if (issue[0] != '\0') {
                fprintf(stderr, " (%s)", issue);
            }
            fprintf(stderr, "\n");
        ...
        printf("  bindings ok: blocks=%d channels=%d heads=%d head_dim=%d in=%d out=%d cond=%d\n",
            weights.n_blocks,
